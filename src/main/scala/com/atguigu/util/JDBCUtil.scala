...
object JDBCUtil {
  ...
  def init(): DataSource = {
    ...
    val properties = new Properties() //配置
    ...
    properties.setProperty("maxActive", config.getProperty("jdbc.datasource.size")) //连接数

    DruidDataSourceFactory.createDataSource(properties) //德鲁伊工厂类创建连接池
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    //5 从结果集里获取数据
    while (resultSet.next()) {
      println("111:" + resultSet.getString(1)) //取出第一列
    }
